...
from liga_predict.config import OUT_DIR  # noqa: E402
...
def _read_with_fallback(preferred: str, fallback: str) -> pd.DataFrame:
    ...
    use = path.exists() and (not alt.exists()
                             or path.stat().st_mtime >= alt.stat().st_mtime - 1)
    return pd.read_csv(path if use else alt)
...
def main():
    ...
    fig.suptitle("La Liga 2026-27 — model predictions", x=0.09, y=0.98,
                 ha="left", fontsize=16, fontweight="bold", color=INK)
    fig.text(0.09, 0.945, "Elo + Poisson model on 26 seasons of La Liga + Segunda data, "
    ...
    ax_title.set_xlim(0, max(0.56, 1.18 * tp[["p_champion", "p_title_market"]].max().max()))
    ax_title.set_xticks([])
    style(ax_title)
    ...
    # --- Next matchday outcome probabilities (100% stacked) ---
    unfinished = matches[~matches["finished"]]
    next_gw = int(unfinished["gameweek"].min()) if not unfinished.empty else int(matches["gameweek"].max())
    gw1 = matches[matches["gameweek"] == next_gw].sort_values("kickoff_utc")[::-1]
    ...
    ax_gw1.set_title(f"Matchday {next_gw} — outcome probabilities (market-calibrated)",
                     loc="left", fontsize=11, fontweight="bold", pad=10)
