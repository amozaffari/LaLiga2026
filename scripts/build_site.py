...
from liga_predict.config import OUT_DIR, PROJECT_ROOT  # noqa: E402
from liga_predict.history import MATCH_LOG, scoreboard  # noqa: E402
...
def projection_rows(table: pd.DataFrame) -> str:
    ...
    for pos, r in enumerate(table.itertuples(), start=1):
        cls = ' class="cutline"' if pos in (4, 17) else ""  # CL places / relegation line
        rows.append(
...
def history_section() -> str:
    ...
    if MATCH_LOG.exists():
        ...
        if not played.empty:
            ...
            parts.append(f"""
  <h3 style="font-size:1rem;margin:1.2rem 0 0.5rem">Matchday {last_gw} —
...
def main():
    def read_pref(preferred, fallback):
        # Market-calibrated file only if it is at least as fresh as the pure one.
        p, alt = OUT_DIR / preferred, OUT_DIR / fallback
        use = p.exists() and (not alt.exists()
                              or p.stat().st_mtime >= alt.stat().st_mtime - 1)
        return pd.read_csv(p if use else alt), use
  ...
  <h2>Matchday {gw} — outcome probabilities</h2>
  <div class="legend">
...
<title>La Liga 2026-27 Predictions</title>
<style>{CSS}</style>
</head>
<body>
<main>
  <h1>La Liga 2026-27 — predictions</h1>
  <p class="sub">Elo + Poisson model on 26 seasons of La Liga + Segunda data · {calib_note}
  ...
  <img class="dash" src="predictions_2026_27.png"
       alt="Prediction dashboard: expected points, title and relegation probabilities, next-matchday outcome probabilities">
{fixtures_section}
...
{history_section()}
  ...
  <footer>
    Data: <a href="https://www.football-data.co.uk/spainm.php">football-data.co.uk</a> ·
    <a href="https://www.espn.com/soccer/league/_/name/esp.1">ESPN</a> ·
    <a href="https://fixturedownload.com/results/la-liga-2026">fixturedownload</a> ·
    <a href="https://biwenger.as.com/">Biwenger</a> ·
    <a href="https://polymarket.com/">Polymarket</a> ·
    code on <a href="https://github.com/amozaffari/LaLiga2026">GitHub</a>.<br>
