...
Market prices under that floor are treated as "about 1%", not as their literal
value: in a two-horse league like La Liga, a club priced at 0.4% for the title
is not being told it is a mid-table side, only that it is not a contender.
    ...
    for it in range(iterations):
        ...
        for team, p_mkt in market.items():
            ...
            if max(p_mkt, p_sim) < MIN_INFORMATIVE_P:
                continue
            target = max(p_mkt, MIN_INFORMATIVE_P)  # sub-1% prices are just "no"
            step = LEARNING_RATE * math.log(target / max(p_sim, floor))
