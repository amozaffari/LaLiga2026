...
def cmd_predict(args):
    ...
    gw = args.matchday or int(upcoming["gameweek"].min())
...
def cmd_squad(args):
    ...
    with pd.option_context("display.float_format", lambda v: f"{v:.0f}"):
        print("\nSquad strength (top-15 expected fantasy points; match horizon "
    ...
    if args.team:
        ...
        if team.empty:
            ...
            return
        cols = ["name", "position", "price", "pts_prev", "pts_now", "form_pts",
...
def cmd_markets(args):
    ...
    for r in sel.itertuples():
        ...
        rows.append(row)
    print(f"\nMatchday {gw} — model vs Polymarket (de-vigged):")
    with pd.option_context("display.float_format", lambda v: f"{v:.2f}"):
        print(pd.DataFrame(rows).to_string(index=False))


def cmd_transfers(args):
    from .squad_model import current_roster

    roster = current_roster()
    new = roster[(roster["pts_prev"] <= 0) & (roster["price"] > 0)].copy()
    new = new.sort_values("value_season", ascending=False)
    per_club = (new.groupby("team_name")
                .agg(newcomers=("name", "size"),
                     est_pts=("value_season", "sum"),
                     market_value_m=("price", lambda s: s.sum() / 1e6))
                .round(0).astype(int)
                .sort_values("est_pts", ascending=False).reset_index())
    unavailable = (roster[roster["status"] != "ok"].groupby("team_name").size()
                   .rename("unavailable"))
    per_club = per_club.merge(unavailable, left_on="team_name", right_index=True,
                              how="left").fillna({"unavailable": 0})
    per_club["unavailable"] = per_club["unavailable"].astype(int)
    print("Players new to La Liga this season by club (no La Liga points last season:")
    print("signings from abroad, promoted-club squads, academy debuts). Their value")
    print("is estimated from Biwenger market price; est_pts is on the fantasy-points scale:\n")
    print(per_club.to_string(index=False))
    print(f"\nMost valuable newcomers (top {args.top}):")
    cols = ["name", "team_name", "position", "price", "value_season", "status"]
    with pd.option_context("display.float_format", lambda v: f"{v:,.0f}"):
        print(new.head(args.top)[cols].to_string(index=False))


def main():
    parser = argparse.ArgumentParser(prog="liga_predict",
                                     description="La Liga match prediction pipeline")
    ...
    p = sub.add_parser("predict", help="probabilities + weather for a matchday")
    ...
    p = sub.add_parser("transfers", help="players new to La Liga this season, per club")
