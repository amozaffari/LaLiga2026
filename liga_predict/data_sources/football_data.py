...
Two divisions are kept: SP1 (La Liga) and SP2 (Segunda División), so promoted
clubs carry an Elo earned from real second-tier matches.
"""
...
def fetch_all(force: bool = False) -> None:
    ...
    file is always re-downloaded (it grows every matchweek) and tolerated as
    missing before the season's first results are published (football-data
    answers 404, or a redirect to another league's file, until then).
    """
    ...
    for season in SEASONS:
        ...
        for div in DIVISIONS:
            ...
            if resp.status_code in (301, 302, 404) and current:
                print(f"{season}/{div} not published yet — skipping")
                continue
            resp.raise_for_status()
            if not resp.content.lstrip().startswith(b"\xef\xbb\xbfDiv") and \
                    not resp.content.lstrip().startswith(b"Div"):
                if current:
                    print(f"{season}/{div} not published yet — skipping")
                    continue
                raise RuntimeError(f"unexpected content for {url}")
...
def _read_one(path) -> pd.DataFrame:
    ...
    out["HomeTeam"] = out["HomeTeam"].map(canonical_team)
    out["AwayTeam"] = out["AwayTeam"].map(canonical_team)
...
def load_matches() -> pd.DataFrame:
    ...
    frames = []
    for path in sorted(RAW_DIR.glob("*_SP*.csv")):
        df = _read_one(path)
        if not df.empty:
            frames.append(df)
    if not frames:
        raise FileNotFoundError("No raw data found — run `python -m liga_predict fetch` first.")
